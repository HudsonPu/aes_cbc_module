...
int main(int argc, char *argv[]) {
  ...
  if(argc != 3)
  {
    fprintf(stderr, "Usage: encrypt_binary [in_filename] [out_filename]\n");
    exit(1);
  }
  ...
  pt_fd = open("/dev/vencrypt_pt", O_WRONLY | O_NONBLOCK);
  if (pt_fd < 0) {
    fprintf(stderr, "Open /dev/vencrypt_pt failed! \n");
    exit(1);
  }
  ct_fd = open("/dev/vencrypt_ct", O_RDONLY | O_NONBLOCK);
  if (ct_fd < 0) {
  ...
  while(!feof(in_fp)){
    ...
    n_write += write(pt_fd, &buffer, n_trans);
  }

  while(n_trans = read(ct_fd, &buffer, sizeof(buffer)))
  {
